...
const Navbar = ({ navlinks }) => {
  ...
  return (
    <>
      ...
      `}>
        <nav className='flex items-center justify-between my-container'>
          <NavLink to={`/`} className="flex items-center">
            <img src={logo} alt='logo/img' className='w-22 h-9 object-fill' />
          </NavLink>
          <ul className='flex items-center lg:hidden gap-7'>
            {navlinks?.map((val, i) => (
              <li key={i}><NavLink to={val.path} className="text-lg text-slate-900">{val.link}</NavLink></li>
            ))}
          </ul>
          <ul className='flex items-center lg:hidden'>
            <li>
              <a href='https://wa.me/+6282375178720'>
                <button type='button' className='button-emrald px-7 text-base'>Whatsapp</button>
              </a>
            </li>
          </ul>
